...
function computeScoringLeader(season){
  const totals = {};
  Object.values(season.schedule || {}).forEach(games => {
    games.forEach(g => {
      if(typeof g.awayScore === 'number'){
        if(!totals[g.away]) totals[g.away] = { team: g.away, owner: g.awayMgr, points: 0 };
        totals[g.away].points += g.awayScore;
      }
      if(typeof g.homeScore === 'number'){
        if(!totals[g.home]) totals[g.home] = { team: g.home, owner: g.homeMgr, points: 0 };
        totals[g.home].points += g.homeScore;
      }
    });
  });
...
function renderSeasonDetail(containerId){
  ...
  const scoringLeader = computeScoringLeader(s);
  const scoringLine = scoringLeader ? `<div class="scoring-leader-line">
    <span class="pill">SCORING TITLE</span>
    <span class="scoring-leader-name">${scoringLeader.owner}</span>
    <span class="scoring-leader-team">— ${scoringLeader.team}</span>
    <span class="scoring-leader-pts">${scoringLeader.points.toFixed(1)} pts</span>
  </div>` : '';
